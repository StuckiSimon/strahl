...
To date, there are reports of stability issues with WebGPU. This includes crashes of the browser, but on macOS using Chrome system-wide crashes have been observed with faulty WebGPU code. Such issues can look like shown in \autoref{fig:webgpu-crash}. This is likely due to the early stage of the technology and the complexity of the underlying system. As implementations mature, these issues are likely to be resolved.
...
The integration of the \gls{OpenPBR} surface shading model enables realistic material representation. As it is a new standard and no stable version has been released when first implementing the path tracer, aligning it with the standard was challenging. As the standard has now reached a stable version, it is easier to align the implementation with the standard. The use of \gls{PBR} and the latest industry standards is beneficial, but few open-source projects have implemented the standard to date.

\section{Future Work}

In order to accomodate for other use cases, the renderer could be extended in various ways. The following section outlines possible future work.
