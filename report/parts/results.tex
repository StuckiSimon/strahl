...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
      ...
      \node[core, below left=2cm and 1cm of intersection] (shader) {Surface Shading};
      \node[core, right= 3cm of shader] (russian) {Russian Roulette};
      \node[pre, right= 1cm of intersection] (terminate) {Termination};
      
  
      \draw[-latex, thick] (ray) -- node[midway, above] {ray} (intersection);
      \draw[-latex, thick] (intersection) -- node[midway, right] {surface} (shader);
    ...
    \end{tikzpicture}
    \caption{High-Level workflow of path tracing core routine.}
