...
Euclidean vectors are fundamental in computer graphics and are generally defined by a magnitude and a direction. In a three-dimensional space, a vector can be defined as $v = (x, y, z)$. This definition can be used to represent points in space called vertices as well as directions. The magnitude, or length, of the vector can be calculated using the Euclidean norm:
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.36\textwidth]{resources/camera-frustum.png}
...
This can be used to assess the quality and reliability of measurements, such as those required for benchmarking.
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Bidirectional reflection models for two different types of surfaces with green arc indicating the direction of the largest contribution to radiance.}
...
\begin{itemize}
  ...
  \item{Reflection}
  \item{Color Bleeding} — A special type of reflection where a surface is colored by reflection of colored light.
  \item{Ambient Occlusion} — A special type of shadow casting, where light transport as impacted by nearby objects is considered. This is frequently encountered on objects with complex topology.
  \item{Refraction}
  \item{Caustics} — An effect produced by reflection or refraction of light.
\end{itemize}
...
This section highlights several techniques that can be used to simulate global illumination within a rasterization-based renderer. These techniques extend the graphics pipeline with additional steps such as post-processing effects or advanced rendering methods. Various methods have been developed, including pre-baked shadow, environment \cite{greene1986environment}, and light maps; screen space reflections (SSR) \cite{screenSpaceReflectionsStackowiak}; screen space ambient occlusion (SSAO) \cite{bavoil2008ssao}; and screen space directional occlusion (SSDO) \cite{ritschel2009ssdo}. These approaches differ in how they need to be set up. Understanding the inner workings aids in understanding their strengths and limitations.
...
Shadow maps are employed to store shadow information for a scene based on the available light sources. These shadows are pregenerated before being employed in the rendering process. While this technique is suitable for static scenes, there are limitations in dynamic lighting situations and scenes where different parts of a product cast shadows on one another.
...
Mathematically, the \gls{MIS} estimator can be defined as:
...
These methods constitute the core of a path tracer. By combining \gls{BVH} for intersection testing, the \gls{MIS} scheme for sampling, and leveraging Russian roulette for probabilistic path termination, Monte Carlo integration of the renderer equation is possible. These methods enable performant, robust light transport to implement ray tracing and are commonplace in other ray tracers.
...
Some of the characteristics to consider when choosing a suitable \gls{RNG} for a path tracer are:
...
One option for such a pseudorandom generator is using Xorshifts, such as described by Marsaglia \cite{marsaglia2003xorshift}. The algorithm can be defined as shown in \autoref{code:xorShift}. $x$ is the state of the \gls{RNG} and $a$, $b$, and $c$ are constants that are chosen to achieve good statistical properties. Frequent choices for these constants are $a = 13$, $b = 17$, and $c = 5$.
...
The Mersenne Twister \cite{rngMersenneTwister} is an alternative \gls{RNG}. It is a pseudorandom number generator that has a long period and good statistical properties. However, it is slower than Xorshifts \cite{o2014pcg}. More recently, \fGls{PCG}{\e{Permuted Congruential Generator}, a family of random number generators.} has been proposed as an alternative to these options. \gls{PCG} is a family of fast generators that is well-suited for ray tracing use.
...
\begin{figure}[H]
  ...
  \begin{subfigure}[t]{0.3\textwidth}
    \centering
    \includegraphics[width=0.78\textwidth]{resources/tonemapping-none.png}
    \caption{No tone mapping.}
    \label{fig:tone-mapping-none}
  \end{subfigure}
  \hfill
  \begin{subfigure}[t]{0.3\textwidth}
    \centering
    \includegraphics[width=0.78\textwidth]{resources/tonemapping-khronos-pbr-neutraltone.png}
    \caption{With tone mapping.}
...
This section provides an overview of the technology used to render 3D scenes. It will introduce the hardware and software used in the field of computer graphics with a focus on technology relevant to this thesis by introducing \gls{WebGPU} and its core concepts.

\paragraph{Parallel Processing}

Sequential processing is limited in terms of performance. As the amount of data to process grows, the time to process the data grows linearly. In 1966, Flynn introduced a classification system for computer architectures based on the instruction streams and data streams that can be processed in parallel, including the definition of single instruction, multiple data (\gls{SIMD}) \cite{flynnTaxonomy,flynnTaxonomy2}. Computer graphics exhibits a large number of problems that can be parallelized. As early as 1970, Watkins described an algorithm for computer graphics leveraging a specialized processor for parallel processing \cite{surfaceAlgorithmProcessor}. Further research, such as Chap, a \gls{SIMD} graphics processor proposed in 1984 by Levinthal and Porter as part of Lucasfilm, introduced a special-purpose processor executing a single instruction on multiple data points \cite{chapSIMDgpu}.

\paragraph{GPU}

In the 1980s, Silicon Graphics introduced so-called superworkstations \cite{sigWorkstation}. These workstations were special-purpose computers optimized for computer graphics. Based on these developments, the first consumer graphics processing unit (\gls{GPU}), the GeForce~256 produced by NVIDIA \cite{evolutionOfGPU}, was developed in the 1990s. The \gls{GPU} is a specialized processor that can be installed on a standard computer and is well-suited for workloads that require parallel processing. While \gls{CPU} parallelization on consumer hardware is generally limited to a few cores, modern \glspl{GPU} have thousands of compute units that enable \gls{SIMD} and, more commonly, single instruction, multiple threads (SIMT). Compared to \gls{SIMD}, which operates on a single processor, \gls{SIMT} extends upon \gls{SIMD} and operates on multiple threads.

Nowadays, \glspl{GPU} are prevalent in consumer hardware such as smartphones, tablets, laptops, and desktops, and their use case is not limited to computer graphics. \glspl{GPU} are used in various applications, such as machine learning (\gls{ML}), scientific computing, and data processing. Devices that do not have a discrete \gls{GPU} often use an integrated \gls{GPU}, which is part of the \gls{CPU} chip.

Due to computational complexity, ray tracing techniques have been limited to offline rendering for a long time. Early ray tracers such as \gls{BMRT} were developed, focusing on leveraging the \gls{CPU} for computations. The introduction of \glspl{GPU} has led to further research focused on optimizing the performance of ray tracing techniques. For instance, techniques for \gls{BVH} construction on the \gls{GPU} instead of the \gls{CPU} have been developed \cite{lauterbach2009GPUbvh}. More recent advancements, such as reservoir-based spatio-temporal importance resampling (\gls{ReSTIR}) \cite{restir} and its subsequent improvements \cite{restirAdvancements,restirGeneralized,restirArea}, have significantly accelerated ray tracing. This progress has enabled real-time ray tracing on consumer hardware.

Modern \gls{GPU} hardware offers support for hardware-accelerated ray tracing. Examples include NVIDIA RTX \cite{nvidiaRtxRayTracing} and Apple \gls{Metal} on M3 \cite{appleM3GpuAdvancements}. These examples demonstrate an increasing trend towards hardware-accelerated ray tracing in recent years. However, many \glspl{GPU} and \glspl{API}, including \gls{WebGPU}, do not yet support hardware-accelerated ray tracing.

\paragraph{Parallelization Architecture}
\label{sec:parallelization-architecture}

\glspl{GPU} are designed to process data in parallel using \gls{SIMT} architecture. The underlying constraints of the hardware differ from those encountered in \glspl{CPU} and must be considered while implementing. This includes optimizing memory access, which is typically slower than computation \cite{surveyOnOptimizationTechniquesForGPU}. Another consideration is control flow divergence. A single instruction is executed on multiple threads, each operating on independent data. The multiple threads are grouped into so-called warps at the hardware level. As only a single instruction is executed per warp, the threads must be synchronized. If one thread in a warp takes a different path, e.g., due to conditional branching, all threads in the warp must wait until the divergent thread completes. This divergence in control flow can lead to performance degradation, as demonstrated by Laine et al. \cite{laine2013megakernels}. When using complex material workflows with highly divergent control flow, the performance can be significantly impacted. One option to address this issue is to split the shaders from a single monolithic shader, known as a megakernel, into multiple smaller kernels. However, this approach can lead to additional overhead due to the increased coordination efforts, manifested as additional global memory and synchronization mechanisms.

\paragraph{Common Strategies}
\label{sec:commonGpuStrategies}

Specific basic definitions have established themselves as standard practices for graphics \glspl{API}. The term shader describes a program that runs on the \gls{GPU}. For the graphics pipeline in its basic form, two shaders are executed in sequence:

\begin{itemize}
  \item{Vertex Shader} — The vertex shader is responsible for transforming the vertices of the geometry into clip space. The output of the vertex shader is a set of vertices that are then rasterized.
...
In addition to the graphics pipeline, many graphics \glspl{API} also support custom code, which does not need to be used exclusively for rendering. This code can be used for general-purpose computing and is generally called \gls{GPGPU}. The corresponding compute pipeline consists of a single stage where input and output can be configured. Shaders that are used for \gls{GPGPU} are often called compute shaders.

\subsection*{OpenGL}

\gls{OpenGL} is an \gls{API} for rendering 3D graphics. After its introduction in 1992, it was widely adopted. Subsequently, the standard has been ported to other platforms and has been extended with new features. The \gls{API} has its own shading language called \gls{OpenGL} Shading Language (\gls{GLSL}). To date, \gls{OpenGL} is still widely used in the industry, but it has been replaced by more modern \glspl{API} in recent years.
...
\begin{itemize}
  ...
  \item{\gls{wgpu}} \cite{wgpuImplementation} — A Rust library that provides a \gls{WebGPU} implementation. It is used in Firefox and \fGls{Deno}{a JavaScript runtime, alternative to Node.js.}.
...
Every compute pipeline, or render pipeline, requires an entry point, as shown in \autoref{code:computePipelineJs}. This entry point can then be defined as shown in \autoref{code:computePipelineWgsl}. Note that the return type definition is optional for functions. Attributes such as \texttt{@compute} are used to provide additional information to the compiler. In this case, it denotes the entry point for the compute pipeline and is mandatory as it enables the usage of specific attributes such as \texttt{@builtin}, which may only be used on entry points.
...
\begin{itemize}
  \item{Uniforms} — Uniform buffers are optimized for data that is read-only on the \gls{GPU} and is the same for all vertices or fragments. Such information could be the projection matrix.
  ...
  \item{Index} — Buffers that contain indices for the vertices.
\end{itemize}
...
\begin{figure}[H]
  ...
  \resizebox{0.8\textwidth}{!}{
    ...
    \begin{tikzpicture}[
      ...
      \node[unused, below=0.3cm of material] (texture) {texture};
...
\begin{itemize}
  ...
  \item{Autodesk Standard Surface} \cite{autodeskStandardSurface} — Developed by Autodesk, the goals are similar to Disney Principled \gls{BRDF} in that the parameter set is kept small in order to be easier to use. The model is developed as an open standard with a reference implementation in \fgls{OSL}{\e{Open Shading Language}, a shading language for production global illumination renderers maintained by the Academy Software Foundation.}. However, the standard has not seen much development in recent years.
...
One core tenet of \gls{OpenPBR} is energy conservation. This means that the \gls{BSDF} must not reflect more light than it receives. The structure consists of multiple layers to describe the material as visualized in \autoref{fig:openPBR}. The layers are:
...
\begin{figure}[H]
  ...
  \resizebox{0.95\textwidth}{!}{
    ...
    \end{tikzpicture}}
  \caption{The different layers of the \gls{OpenPBR} surface shading model. The model supports various features such as metallic, glossy, and diffuse reflection renderings that are important for industrial use cases.}
  \label{fig:openPBR}
\end{figure}
...
While there is a large variety of material description standards, \gls{OpenPBR} is a promising candidate. The focus on surface shading is well-suited for the use case of this thesis. Compared to other standards, it has significant industry backing and is being adopted by major parties in the computer graphics industry, such as Adobe, Autodesk, and NVIDIA. The goal of further extension of the standard is a positive indicator. Its design principles, such as focusing on physical aspects instead of specifying implementation details, and the \gls{uber shader} approach make it well-suited for real-time rendering.
