...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
      \begin{axis}[
          ...
          \addplot[name path=A, blue, thick] {x/2.5+((x^2)/2)} node[above,pos=1] {$f(x)$};
          \path[name path=B] (axis cs:0,0) -- (axis cs:2,0);
          \addplot[blue!50, opacity=0.3] fill between[of=A and B, soft clip={domain=0.5:1.5}];
      \end{axis}
  \end{tikzpicture}
...
The Bachmann-Landau notation, or more specifically, the Big O notation, is used to describe the behavior of a function as the input size grows. The notation is used to describe the upper bound of the function. For example, if a function $f(n)$ is $O(n^2)$, it means that the function grows at most quadratically with the input size $n$. The Landau notation is used to describe the growth of algorithms in terms of run time or space requirements.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
      \begin{axis}[
          ...
          \addplot[blue, thick] {x} node[above,pos=1] {$O(n)$};
          \addplot[green, thick] {x/2} node[below,pos=1] {$f(x)$};
          \addplot[red, thick, domain=0:5.5] {(x^2)/2} node[above,pos=1] {$g(x)$};
      \end{axis}
  \end{tikzpicture}
...
The Monte Carlo method is a statistical technique which utilizes random sampling to approximate the solution to a problem. The basic form can be defined as:
