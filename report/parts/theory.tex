...
\glspl{GPU} are designed to process data in parallel using \gls{SIMT} architecture. A single instruction is executed on multiple threads which operate on independent data. The multiple threads are grouped into warps on hardware level. As only a single instruction is executed on a warp, the threads need to synchronized. If one thread in a warp takes a different path, e.g. due to conditional branching, all threads in the warp need to wait until the thread has finished. This divergence in control flow can lead to performance degradation \cite{laine2013megakernels}.
