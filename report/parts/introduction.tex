...
An instance at EAO involves a specific product family within the so-called series 45. This particular family comprises 14 modules ($n$), each with approximately ten different options ($o$). This results in $10^{14}$ possible configurations ($t$) based on 140 different components. In addition, EAO has many more product series with varying numbers of components and options. This constitutes the primary use case considered for this thesis. The presented work uses EAO engineering models to demonstrate the feasibility and challenges of real-time rendering for highly configurable products. Nevertheless, similar use cases can be observed in other industries.
