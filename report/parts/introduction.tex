...
Since then, various open-source implementations for the web have been developed. While closed-source options exist, this work focuses on widely available open-source variants. The inherent advantages of open-source software lie in the possibility to inspect and modify the code, as well as in permissive license agreements, which often allow for free use in perpetuity. Consequently, closed-source alternatives are not considered in this context. A more detailed comparison of these path tracers is provided in \autoref{sec:comparisonToPriorWork}.
