...
fn writeColor(pixelColor: vec4f, x: i32, y: i32) {
  textureStore(texture, vec2<i32>(x, y), pixelColor);
}
...
fn computeMain(@builtin(global_invocation_id) globalId: vec3<u32>) {
  ...
  //writeColor(output, i32(globalId.x), i32(globalId.y));
  //writeColor(vec4f(1.0), i32(globalId.x), i32(globalId.y));
