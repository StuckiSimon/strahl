...
import { WGPUFullQuadPass } from "./full-oidn-pass.ts";
...
async function denoise(
  { device, adapterInfo }: { device: GPUDevice; adapterInfo: GPUAdapterInfo },
  {
    colorBuffer,
    albedoBuffer,
    normalBuffer,
  ...
  const TZA_URL = "./oidn-weights/rt_hdr_alb_nrm.tza";
  ...
  return new Promise((resolve, reject) => {
    unet.tileExecute({
      ...
      done(finalBuffer, tileData, tile) {
      ...
      if (currentSample < TARGET_SAMPLES && !isHalted()) {
        ...
        if (enableDenoise) {
          ...
          // fixme: use manual texture-converter-pass-shader
          {
            ...
            const dispatchX = Math.ceil(width / maxWorkgroupDimension);
            const dispatchY = Math.ceil(height / maxWorkgroupDimension);
            console.log("dispatch", dispatchX, dispatchY);
            computePass.dispatchWorkgroups(dispatchX, dispatchY);
          ...
          encoder.copyTextureToBuffer(
            ...
            { buffer: textureBuffer, bytesPerRow: width * 4 },
            [width, height],
          );
          device.queue.submit([encoder.finish()]);*/
          ...
          const textureMapped = textureBuffer.getMappedRange();
          const textureData = new Float32Array(textureMapped);
          console.log(tileData, textureData);
          textureData.set(tileData.data, 0);
...

          {
            ...
            encoder.copyBufferToTexture(
              // fixme: this is used to debug the different buffers
